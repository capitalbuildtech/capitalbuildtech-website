...
export default function ProjectCard({ project, index = 0 }) {
  return (
    ...
    >
      <Link to={`/projects/${project.slug}`} aria-label={`Open ${project.title}`}>
        <div className="aspect-[4/3] overflow-hidden bg-brand-ink/10">
          <img
            src={project.image}
            alt={project.imageAlt}
            loading="lazy"
            className="image-cover transition duration-700 ease-premium group-hover:scale-105"
          />
        </div>
        <div className="p-6">
